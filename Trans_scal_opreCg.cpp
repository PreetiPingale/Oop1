/*a) Write C++ program to draw 2-D object and perform following basic a)
...
void transform::accept()
{
	...
	for(int i=0;i<m;i++)
	{
		for(int j=0;j<3;j++)
		{
			if(j>=2)			//cretes matrix ...e.g 3 edges
			a[i][j]=1;			//  x1  y1  1
			else				//  x2  y2  1
			cin>>a[i][j];       //  x3  y3  1
		}
...
int main()
{
	...
	switch(ch)
	{
		case 1: cout<<"\nTRANSLATION OPERATION\n";
				...
				cin>>tx>>ty;
				b[0][0]=b[2][2]=b[1][1]=1;          //    0     1    2
				b[0][1]=b[0][2]=b[1][0]=b[1][2]=0;	// 0  1     0    0
				b[2][0]=tx;							// 1  0		1    0
				b[2][1]=ty;							// 2  tx	ty	 1
				t * b;
				t.object();
		...
		case 2: cout<<"\nSCALING OPERATION\n";
				...
				b[0][0]=sx;								//    0     1    2
				b[1][1]=sy;								// 0 sx     0    0
				b[0][1]=b[0][2]=b[1][0]=b[1][2]=0;      // 1 0	   sy    0
				b[2][0]=b[2][1]=0;						// 2 0		0	 1
				b[2][2] = 1;
				t * b;
				t.object();
				break;
		case 3: cout<<"\nROTATION OPERATION\n";
				...
				cin>>deg;
				theta=deg*(3.14/100);				 //    0     1    2
				b[0][0]=b[1][1]=cos(theta);          // 0 cos   sin   0
				b[0][1]=sin(theta);				   	 // 1 -sin  cos   0
				b[1][0]=sin(-theta);			     // 2	0	 0	  1
